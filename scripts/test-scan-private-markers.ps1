...
$preexistingScannerIsolationRoots = @(
    Get-ChildItem -LiteralPath ([System.IO.Path]::GetTempPath()) `
        -Directory `
        -Filter 'isolated-worktree-pr-flow-git-*' `
...
'@
    ...
    Set-Content -LiteralPath (Join-Path $cleanRoot 'README.md') -Value @(
        ...
        ('Own repo: ' + (('https://github' + '.com/') + 'h8nc4y/isolated-worktree-pr-flow'))
        ('Own clone URL: ' + (('https://github' + '.com/') + 'h8nc4y/isolated-worktree-pr-flow.git'))
    ) -Encoding UTF8
    ...
    $allowedUrl = ('https://github' + '.com/') +
        'h8nc4y/isolated-worktree-pr-flow'
