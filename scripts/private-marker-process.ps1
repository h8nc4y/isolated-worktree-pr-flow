...
function Test-PrivateMarkerGitIsolationRootBoundary {
    ...
    return (
        ...
        $rootName -cmatch '^isolated-worktree-pr-flow-git-[0-9a-f]{32}$'
    )
}
...
function New-PrivateMarkerGitIsolationRoot {
    ...
    try {
        ...
        New-Item `
            ...
            -ErrorAction Stop |
            Out-Null
        $ownerPath = Join-Path $Root '.isolated-worktree-pr-flow-owner'
...
function Assert-PrivateMarkerGitIsolationRootState {
    ...
    try {
        $ownerPath = Join-Path $rootItem.FullName '.isolated-worktree-pr-flow-owner'
        $ownerItem = Get-Item `
